...
#define PU_DEVICE_NUM_MAX          (1000)        //最大接入5000路设备
...
#define ETH_RECV_RATE_MAX          8000          //收包速率限制上限
...
#define DM_LOGIN_NAME_LEN_MAX (64)
#define DM_LOGIN_PWD_LEN_MAX (64)
...
typedef enum ERROR_CODE
{
    ...
    // 控制错误
    PU_ERROR_CODE_ITGT_IMRS_ERR_NOT_CONFIG = 10961,
    PU_ERROR_CODE_ITGT_IMRS_ERR_PARAM_SET = 10962,
    //车道线错误
    ...
    PU_RESULT_ERR_POINT_OUTOF_RANGE,							//检测线的起始点和终结点超出图象区域
    PU_RESULT_ERR_LINE_BECOME_POINT,							//检测线不能设置为点
    PU_RESULT_ITS_ERR_LINE_OUTOF_RANGE,							//检测线、停止线不在车道线或右边界线有效范围内
    PU_RESULT_ERR_LINE_CROSS,									//检查顶点构成的线是否有交叉
...
typedef enum PU_PLATFORM_TYPE
{
	...
	PU_PLATFORM_TYPE_DM_ENABLE,                //DM平台参数，带使能
...
typedef enum PU_ITGT_TYPE
{
    ...
    PU_ITGT_TYPE_NON_CHINA_VEH_RECOGTION,         //海外车牌检测Non-China vehicle recognition 
...
{
    PU_IRC_MODE_COLOR = 0,           //黑白模式
    PU_IRC_MODE_GRAY,                //彩色模式
...
typedef struct PU_AUDIO_INFO
{
    ...
    CHAR szReserved[PU_RESERVE_LEN];                                //szReserved[0]标识采样率 : 0 8K, 1 12K, 2 16K, 3 24K, 4 48K
...
typedef struct PU_PLATFORM_SDK_PARA
{
    ...
    CHAR szReserve[PU_RESERVE_LEN - sizeof(PU_CONNECT_TYPE_E)];
}PU_PLATFORM_SDK_PARA_S, *LPPU_PLATFORM_SDK_PARA_S;

//DM主动注册的地址
typedef struct PU_PLATFORM_DM_PARA
{
	BOOL bEnable;
	PU_SDK_PLATFORM_MODE_E enSdkPlatformMode;
	PU_CONNECT_TYPE_E enConnectType;
	...
	ULONG szBackupPlatformPort;
	CHAR szReserve[PU_RESERVE_LEN];
}PU_PLATFORM_DM_PARA_S, *LPPU_PLATFORM_DM_PARA_S;
...
typedef struct PU_REAL_PLAY_INFO
{
    ...
    CHAR szReserved[PU_RESERVE_LEN];                                //szReserved[22]表示智能分析数据打包格式 0:XML,1:元数据
...
typedef struct PU_ISP_PARTICULAR_PARA
{
    ...
    PU_SLOW_SHUTTER_MODE_S stSlowShutterMode;                   //慢快门开关6
...
{
    ULONG ulIndex;                                  //唯一标识 1-4是基本配置不能删除,0标示不存在
    CHAR szName[PU_TYPICAL_SCENE_NAME_LEN];         //典型场景餐名称名称
...
typedef struct PU_FACE_DETC_PARA
{
    ...
    ULONG ulMinDectFace;        // 最小检测人脸
    ULONG ulMaxDectFace;        // 最大检测人脸
    ULONG ulDectMode;           // 检测模式 0:正常模式
    CHAR  szReserved[PU_RESERVE_LEN];
}PU_FACE_DETC_PARA_S;

typedef struct PU_FACE_TRACKING_PARAMETER
{
    BOOL bFilter;               // 启用跟踪过滤模式
...
typedef struct PU_FACE_DETC_DL_PARA
{
	...
	ULONG ulAlarmTime;          // 告警检测时间
    ULONG ulSensitivity;        //检测灵敏度
    ULONG ulFaceSensitivity;    //人脸检测灵敏度，预留
    ULONG ulBodySensitivity;    //人体检测灵敏度，预留
    ULONG ulFaceKpsSensitivity; //人脸质量过滤灵敏度，预留
    ULONG ulBodyKpsSensitivity; //人体质量过滤灵敏度，预留
...
typedef struct PU_VHD_PARA
{
    ...
    CHAR 			szReserved[PU_RESERVE_LEN - sizeof(BOOL) - sizeof(BOOL)];
...
{
    PU_COMB_IMG0_ORI, /* 第0张抓拍原图 */
    PU_COMB_IMG0_ROI, /* 第0张抓拍抠图 */
    PU_COMB_IMG1_ORI, /* 第1张抓拍原图 */
    PU_COMB_IMG1_ROI, /* 第1张抓拍抠图 */
    PU_COMB_IMG2_ORI, /* 第2张抓拍原图 */
    PU_COMB_IMG2_ROI, /* 第2张抓拍抠图 */
    PU_COMB_IMG2_BUTT,
...
{
    ULONG                       ulChannelId;     //通道号
    PU_ALARM_LINK_PTZ_PRIORITY  enAlarmPriority; //优先级
    CHAR                        szReserved[PU_RESERVE_LEN];
...
typedef struct PU_VA_LINK_AT
{
    ULONG           ulChannelId;    //通道号
    BOOL            bEnable;        //算法使能
    PU_ALARM_TYPE   enAlarmType;    //需要联动自动跟踪的告警
    CHAR            szReserved[PU_RESERVE_LEN];
...
typedef struct PU_MANAGE_3RD_APP_INFO
{
	...
	PU_3RD_APP_BASIC_INFO_S ast3RdAppInfo[PU_ITGT_TYPE_MAX_NUM];   //三方算法App信息
	CHAR szReserved[PU_RESERVE_LEN]; 
...
typedef struct PU_FACE_RECORD
{
    ...
    CHAR szProvince[PU_MAX_RECORD_LEN_16];              // 省级
    CHAR szCity[PU_MAX_RECORD_LEN_16];                  // 地市
...
typedef struct PU_FACE_LIB_IMPORT_S
{
    ...
    CHAR                szImportResult[PU_IMPORT_FILE_LEN_MAX];    //导出结果CSV保存路径，预留
    CHAR                szReserve[PU_RESERVE_LEN];
...
{
    PU_ENABLE_TYPE_E   bEnable;        //使能开关
    ULONG             ulChannelId;
    ULONG             uInterval;      //车流量统计间隔
    CHAR              szReserve[PU_RESERVE_LEN];
...
//device Genetec protocol switch
...
//device Genetec protocol digest authentication switch
...
//获取DM平台参数
_HW_PU_API BOOL __stdcall IVS_PU_GetDMPlatformPara(ULONG ulIdentifyID, LPPU_PLATFORM_DM_PARA_S pstDMPlatformPara);

//设置DM平台参数
_HW_PU_API BOOL __stdcall IVS_PU_SetDMPlatformPara(ULONG ulIdentifyID, LPPU_PLATFORM_DM_PARA_S pstDMPlatformPara);
